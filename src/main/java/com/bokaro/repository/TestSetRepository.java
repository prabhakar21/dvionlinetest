...
public interface TestSetRepository extends JpaRepository<TestSet, Long> {
	...
	@Query("SELECT t FROM User u,CandidateRegistration c,TestSet t,CandidateTestSetConfiguration ct where c.id=ct.candidateId and ct.testSetId=t.id and u.email=c.email and u.userName=?1")
