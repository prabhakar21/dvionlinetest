...
public class CandidateRegistrationServiceImpl implements CandidateRegistrationService {
	...
	@Override
	public CandidateRegistrationDTO create(CandidateRegistrationDTO candidateRegistrationDTO) throws ProgramException {
		CandidateRegistration candidateRegistration = candidateRegistrationMapper.candidateRegistrationDTOToCandidateRegistration(candidateRegistrationDTO);
			...
			candidateRegistration.setDateCreated(LocalDate.now());
	...
	@Override
	public List<CandidateRegistrationDTO> getCandidateDetails(String name, String email, String mobileNumber,String date) {
		List<CandidateRegistration> candidateRegistrationList = candidateCustomRepository.searchCandidate(name,email,mobileNumber,date);
		List<CandidateRegistrationDTO> candidateRegistrationDTOList = candidateRegistrationMapper.candidateRegistrationListToCandidateRegistrationDTOList(candidateRegistrationList);
		for(CandidateRegistrationDTO candidateRegistrationDTO : candidateRegistrationDTOList){
			User user = userRepository.findByEmail(candidateRegistrationDTO.getEmail());
			candidateRegistrationDTO.setUsername(user.getUserName());
	...
	@Override
	public Page<CandidateRegistrationDTO> findAllPageable(Pageable pageable) {
		Page<CandidateRegistration> candidateRegistrations = candidateRegistrationRepository.findAll(pageable);
		List<CandidateRegistrationDTO> candidateRegistrationDTOs = candidateRegistrations.getContent().stream().map(candidateRegistrationMapper::candidateRegistrationToCandidateRegistrationDTO)
				.collect(Collectors.toList());
		return new PageImpl<CandidateRegistrationDTO>(candidateRegistrationDTOs, pageable, candidateRegistrations.getTotalElements());
	}
